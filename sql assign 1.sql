...
select d.department_name,count(e.employee_id) as recent_hires from departments d join employees e using(department_id)
 where e.hire_date>"2020-01-01"
group by department_name;


#q7 Show department names and the number of employees whose salary is more than 60000
select department_name,count(e.employee_id) as employee_total from departments d join employees e 
...
select department_name,avg(salary) as salary_avg from departments d  inner join employees e
 using(department_id)  group by department_name having salary_avg>60000 ;
...
using(department_id)  group by department_name having total_employees>2;
...
using(department_id) where  year(hire_date)="2021" group by department_name  ;
...
select department_name,count(*) as total_employees,max(hire_date) as latest_hiredate from departments d inner join employees e 
...
select department_name,avg(e.salary) as average_salary from departments d  inner join employees e 
using(department_id)  where(e.name) between "A" and "M" group by department_name ;

#Q15 Display count of employees grouped by department and year of hire.
select department_name,count(employee_id) as empl_count,year(hire_date) as hire_year from departments d  inner join employees e 
using(department_id)   group by department_name,hire_year order by department_name,hire_year ;

#Q16 List all departments and how many employees earn more than the average salary of their department.
select department_name, count(employee_id) as total_employee from departments d  inner join employees e 
using(department_id) where salary>(select avg(salary) as avg_salary from employees) group by department_name ;


#Q17 Show each department with the number of employees earning below 60000.
select department_name, count(employee_id) as total_employee from departments d  inner join employees e 
using(department_id) where salary<60000 group by department_name ;

#Q18 Display department names with the total salary of employees who joined before 2020.
select department_name, sum(salary) as total_salary from departments d  inner join employees e 
using(department_id) where year(hire_date)<"2020" group by department_name ;

#Q19 Show department names and the count of employees who joined in each year.
select department_name,count(employee_id) as total_count,year(e.hire_date) as hire_year from departments d  inner join employees e 
using(department_id)  group by year(e.hire_date),department_name ;

select count(employee_id) as total_count,year(e.hire_date) as hire_year from departments d  inner join employees e 
using(department_id)  group by year(e.hire_date) ;

#Q20 For each department, display average salary and label it as ‘High’ if avg > 60000, else ‘Low’.
select d.department_name,avg(salary) as avg_salary ,    CASE 
WHEN avg(e.salary)>60000 THEN "HIGH" ELSE "LOW" 
 end as salary_level from employees e inner join
     departments d using (department_id) group by d.department_name ;
